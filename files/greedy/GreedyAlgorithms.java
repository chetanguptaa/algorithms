...
//        System.out.println(disjointIntervals(new int[][] {{1,4}, {2, 3}, {3, 5}, {6,7}}));
//        System.out.println(Arrays.toString(largestPermutation(new int[] {3,2,4,1,5}, 3)));
//        System.out.println(meetingRooms(new int[][] {{5, 10}, {15, 20}, {0,30}}));
    ...
    static int meetingRooms(int[][] meetingIntervals) {
        ...
        for(int i = 1; i < meetingIntervals.length; i++) {
            if(meetingIntervals[i][0] < meetingIntervals[i-1][1]) count++;
        }
        return count;
    }
    static int distributeCandy(int[] ratings) {
        ...
        for (int[] pair : data) {
            int i = pair[1];
            if (i > 0 && ratings[i] > ratings[i - 1]) candies[i] = Math.max(candies[i], candies[i - 1] + 1);
            if (i < n - 1 && ratings[i] > ratings[i + 1]) candies[i] = Math.max(candies[i], candies[i + 1] + 1);
        }
        int totalCandies = 0;
        for (int count : candies) totalCandies += count;
        return totalCandies;
    }
}