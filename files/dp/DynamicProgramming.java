...
//        System.out.println(canConstructDp("skateboard", new String[] {"bo", "rd", "ate", "t", "ska", "sk", "boar"}, new HashMap<>()));
...
//        System.out.println(allConstructDp("abcdef", new String[] {"ab", "abc", "cd", "def", "abcd", "ef", "c"}, new HashMap<>()));
    }
    ...
    static int gridTravellerDp(int numOfRow, int numOfCol, Map<List<Integer>, Integer> memo) {
        ...
        if(memo.containsKey(listReverse)) return memo.get(listReverse);
        if( numOfRow == 0 || numOfCol == 0 ) return 0;
        if( numOfRow == 1 && numOfCol == 1 ) return 1;
    ...
    static List<Integer> bestSumDp(int targetSum, int[] numbers, Map<Integer, List<Integer>> memo) {
        ...
        for(int num : numbers) {
            ...
            if(remainderResult != null) {
                remainderResult.add(num);
                if(ans == null || remainderResult.size() < ans.size()) ans = remainderResult;
            }
        }
    ...
    static List<List<String>> allConstruct(String target, String[] wordBank) {
        ...
        for (String word : wordBank) {
            if (target.startsWith(word)) {
                ...
                for (List<String> way : suffixWays) way.add(0, word);
