...
public class Knapsack01 {
    ...
    private static int helper(int[] wt, int[] val, int cap, int n, int[][] matrix) {
        ...
        if (matrix[n][cap] != -1) return matrix[n][cap];
        if (wt[n - 1] <= cap) {
            matrix[n][cap] = Math.max(val[n - 1] + helper(wt, val, cap - wt[n - 1], n - 1, matrix), helper(wt, val, cap, n - 1, matrix));
        } else matrix[n][cap] = helper(wt, val, cap, n - 1, matrix);
        return matrix[n][cap];
    }
    ...
    public static int knapsackBottomUp(int[] wt, int[] val, int cap, int n) {
        ...
        for (int i = 1; i < n + 1; i++) {
            for (int j = 1; j < cap + 1; j++) {
                if (wt[i - 1] <= j) matrix[i][j] = Math.max(val[i - 1] + matrix[i - 1][j - wt[i - 1]], matrix[i - 1][j]);
